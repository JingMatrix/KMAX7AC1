...
Le coalescent de Kingman est un processus markovien à temps continu qui avla forme d'un arbre généalogique. Pour $n$ individus initiaux, on remonte les lignées ancestrales de cet individu, et à tout instant deux lignées quelconques coalesent à taux $c$ pour n'en former qu'une seule.
\begin{enumerate}
...
\begin{enumerate}
    \item
		La question demande la loi de $T_n$, qui dans l'énoncé est le premier temps où le nombre de lignées vaut $n$. Par convention, le processus démarre à $t=0$ avec $n$ lignées, donc $T_n = 0$. La question vise en réalité à trouver la loi du \emph{temps d'attente jusqu'à la première coalescence}, que nous avons noté $W_n$.

    Quand il y a $n$ lignées, il y a $\binom{n}{2}$ paires de lignées distinctes. Chaque paire est susceptible de coalescer. Selon le modèle, chaque paire coalesce à un taux constant $c$. On peut donc modéliser cet événement comme $\binom{n}{2}$a ``réveils'' indépendants, où chaque réveil sonne à un temps $X_{ij} \sim \mathcal{E}(c)$ pour la paire $(i,j)$.

    La première coalescence se produit au moment où le premier de ces réveils sonne. Le temps d'attente $W_n$ est donc le minimum de $\binom{n}{2}$ variables aléatoires exponentielles i.i.d. de paramètre $c$:
    \[ W_n = \min \{ X_{ij} : 1 \le i < j \le n \}. \]
    Une propriété fondamentale de la loi exponentielle (qui est une généralisation du "lemme des réveils") est que le minimum de $N$ variables aléatoires indépendantes $X_i \sim \mathcal{E}(\lambda_i)$ suit une loi exponentielle de paramètre $\sum \lambda_i$.

    Ici, nous avons $N = \binom{n}{2}$ variables, et toutes ont le même paramètre $\lambda_i = c$. Le paramètre de la loi de $W_n$ est donc la somme des paramètres :
    \[ \lambda_{W_n} = \sum_{k=1}^{\binom{n}{2}} c = c \binom{n}{2}. \]
    Ainsi, le temps de la première coalescence suit une loi exponentielle de paramètre $c\binom{n}{2}$.

    \item
		La question demande la loi de $T_k - T_{k+1}$. Selon notre notation, $T_k$ est le temps absolu où il reste $k$ lignées, et $T_{k+1}$ est le temps absolu où il en reste $k+1$. La différence $T_k - T_{k+1}$ est donc précisément le \emph{temps d'attente pendant lequel il y a $k+1$ lignées}, c'est-à-dire $W_{k+1}$.

    Grâce à la propriété de Markov du processus (ou l'absence de mémoire de la loi exponentielle), le processus ``redémarre'' à chaque coalescence. Quand il reste $k+1$ lignées, nous sommes dans la même situation qu'à la question 1, mais en remplaçant $n$ par $k+1$.

    Le nombre de paires de lignées est donc $\binom{k+1}{2}$, et le taux de la prochaine coalescence est $c\binom{k+1}{2}$. Par conséquent :
    \[ T_k - T_{k+1} = W_{k+1} \sim \mathcal{E}\left(c\binom{k+1}{2}\right). \]

    \item
    Le temps absolu $T_k$ pour atteindre $k$ lignées en partant de $n$ est la somme des temps d'attente successifs. En partant de $t=0$ avec $n$ lignées, on attend $W_n$ pour atteindre $n-1$ lignées, puis $W_{n-1}$ pour atteindre $n-2$ lignées, et ainsi de suite jusqu'à $W_{k+1}$.
    \[ T_k = W_n + W_{n-1} + \dots + W_{k+1} = \sum_{j=k+1}^{n} W_j. \]
    D'après la question précédente, nous savons que $W_j = T_{j-1} - T_j \sim \mathcal{E}(c\binom{j}{2})$.
    Les temps d'attente $W_j$ sont indépendants en raison de la propriété de Markov du processus.

    On utilise la propriété de changement d'échelle de la loi exponentielle : si $E \sim \mathcal{E}(1)$, alors la variable $X = E/\lambda$ suit une loi $\mathcal{E}(\lambda)$.
    On peut donc écrire chaque $W_j$ comme :
    \[ W_j \overset{(d)}{=} \frac{E_j}{c \binom{j}{2}}, \]
    où les $E_j$ sont une suite de variables aléatoires i.i.d. de loi $\mathcal{E}(1)$. L'égalité est une égalité en distribution.

    En substituant dans la somme, on obtient :
    \[ T_k \overset{(d)}{=} \sum_{j=k+1}^{n} \frac{E_j}{c \binom{j}{2}}. \]

    \item
    $T_\mathrm{MRCA}$ (Most Recent Common Ancestor) est le temps $T_1$, c'est-à-dire le temps jusqu'à ce qu'il ne reste qu'une seule lignée. On applique la formule de la question 3 avec $k=1$.
    \[ T_\mathrm{MRCA} = T_1 = \sum_{j=2}^{n} W_j. \]
    Par linéarité de l'espérance :
    \[ \E(T_\mathrm{MRCA}) = \sum_{j=2}^{n} \E(W_j). \]
    L'espérance d'une variable $\mathcal{E}(\lambda)$ est $1/\lambda$. Donc $\E(W_j) = \frac{1}{c \binom{j}{2}}$.
    \[ \E(T_\mathrm{MRCA}) = \sum_{j=2}^{n} \frac{1}{c \binom{j}{2}} = \frac{1}{c} \sum_{j=2}^{n} \frac{1}{\frac{j(j-1)}{2}} = \frac{2}{c} \sum_{j=2}^{n} \frac{1}{j(j-1)}. \]
    On utilise la décomposition en éléments simples $\frac{1}{j(j-1)} = \frac{1}{j-1} - \frac{1}{j}$. La somme devient une somme télescopique :
    \begin{align*}
    \sum_{j=2}^{n} \left(\frac{1}{j-1} - \frac{1}{j}\right) &= \left(\frac{1}{1} - \frac{1}{2}\right) + \left(\frac{1}{2} - \frac{1}{3}\right) + \dots + \left(\frac{1}{n-1} - \frac{1}{n}\right) \\
    &= 1 - \frac{1}{n}.
    \end{align*}
    En rassemblant les termes, on obtient le résultat final :
    \[ \E(T_\mathrm{MRCA}) = \frac{2}{c}\left(1 - \frac{1}{n}\right). \]

    \item \textbf{TP-} Les solutions pour le TP se trouvent dans le script Python.
\end{enumerate}
\end{solution}
